...
ROT_SPEED = 5.0 #prędkość obrotu w radianach
THRUST = 400.0 #przyspieszenie
FRICTION = 150.0 #siła tarcia
MAX_SPEED = 400.0 #max prędkość
BRAKE_FORCE = 400.0 #hamowanie 
...
class Ship:
    ...
    def update(self, dt):
        ...
        elif self.pos.x < 0: self.pos.x = 800
        if self.pos.y > 600: self.pos.y = 0
        elif self.pos.y < 0: self.pos.y = 600
        ...
        if rl.is_key_down(rl.KeyboardKey.KEY_UP) or rl.is_key_down(rl.KeyboardKey.KEY_W):
            ...
            self.vel.x += direction.x * THRUST * dt
            self.vel.y += direction.y * THRUST * dt
        ...
        #tarcie i hamulec
        if speed > 0:
            current_force = BRAKE_FORCE if rl.is_key_down(rl.KeyboardKey.KEY_Z) else FRICTION
    ...
    def draw(self):
        ...
        for v in self.verts:
            ...
            t_verts.append(rl.Vector2(rotated.x + self.pos.x, rotated.y + self.pos.y))
        ...
        for v in self.bar_points:
            ...
            t_bar.append(rl.Vector2(rot_bar.x + self.pos.x, rot_bar.y + self.pos.y))
        ...
        if rl.is_key_down(rl.KeyboardKey.KEY_UP) or rl.is_key_down(rl.KeyboardKey.KEY_W):
            if (int(rl.get_time() * 20) % 2 == 0):
                ...
                for v in f_points:
                    ...
                    t_flame.append(rl.Vector2(rot_f.x + self.pos.x, rot_f.y + self.pos.y))
        ...
        if rl.is_key_down(rl.KeyboardKey.KEY_Z):
            p1 = self.rotate_point(rl.Vector2(-18, 24), self.rotation + math.pi/2)
            p2 = self.rotate_point(rl.Vector2(18, 24), self.rotation + math.pi/2)
            rl.draw_circle_v(rl.Vector2(p1.x + self.pos.x, p1.y + self.pos.y), 3, rl.RED)
            rl.draw_circle_v(rl.Vector2(p2.x + self.pos.x, p2.y + self.pos.y), 3, rl.RED)